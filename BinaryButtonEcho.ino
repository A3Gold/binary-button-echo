...
void loop()
{
    if (digitalRead(binary1) == HIGH)
    {                              // If binary1 is pressed,
        binarySum = binarySum + 1; // add 1 to the sum
    }
    if (digitalRead(binary2) == HIGH)
    {                              // If binary2 is pressed,
        binarySum = binarySum + 2; // add 2 to the sum
    }
    if (digitalRead(binary4) == HIGH)
    {                              // If binary4 is pressed,
        binarySum = binarySum + 4; // add 4 to the sum
    }
    if (digitalRead(binary8) == HIGH)
    {                              // If binary8 is pressed,
        binarySum = binarySum + 8; // add 8 to the sum
    }
    if (digitalRead(binary16) == HIGH)
    {                               // If binary16 is pressed,
        binarySum = binarySum + 16; // add 16 to the sum
    }
    if (digitalRead(binary32) == HIGH)
    {                               // If binary32 is pressed,
        binarySum = binarySum + 32; // add 32 to the sum
    }
    if (digitalRead(binary64) == HIGH)
    {                               // If binary64 is pressed,
        binarySum = binarySum + 64; // add 64 to the sum
    }
    if (digitalRead(binary128) == HIGH)
    {                                // If binary128 is pressed,
